  ...
  <div>
        ...
        <div class="page-title-box">
          <div class="page-title-right">
            <ol class="breadcrumb m-0">
              ...
              <li class="breadcrumb-item active">Giới tính</li>
            </ol>
          </div>
          <h4 class="page-title">Giới tính</h4>
        </div>
      </div>
    </div>
    <!-- end page title -->
    ...
    >
      <el-form
        ...
        label-width="150px"
      >
        <!-- <el-form-item label="Mã Giới tính" prop="MaGioiTinh">
          <el-input
            v-model="formData.MaGioiTinh"
            type="text"
            size="small"
            :readonly="isEditor"
          ></el-input>
        </el-form-item> -->
        <el-form-item label="Tên Giới tính" prop="TenGioiTinh">
          <el-input
            v-model="formData.TenGioiTinh"
            type="text"
            size="small"
          ></el-input>
        </el-form-item>
    ...
    return {
      ...
      formRules: {
        TenGioiTinh: [
          {
            required: true,
            message: "Vui lòng nhập Tên Giới tính",
            trigger: "blur"
          }
        ]
      },
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm("Bạn có chắc chắn muốn xóa?", "Thông báo", {
        ...
        deleteGioiTinh(row.Id).then(data => {
          this.$message({
            type: "success",
    ...
    updateData() {
      this.$refs.formData.validate(valid => {
        if (valid) {
          if (this.isEditor == 0) {
            addGioiTinh(this.formData).then(data => {
              //console.log(data);
              this.getListData();
            });
          } else {
            updateGioiTinh(this.formData.Id, this.formData).then(data => {
              //console.log(data);
              this.getListData();
            });
          }
    ...
    renderData() {
      var _data = this.listData.filter(post => {
        return (
          post.TenGioiTinh.toLowerCase().includes(this.search.toLowerCase())
        );
      });
