  ...
  <div>
        ...
        <div class="page-title-box">
          <div class="page-title-right">
            <ol class="breadcrumb m-0">
              ...
              <li class="breadcrumb-item active">Văn bằng</li>
            </ol>
          </div>
          <h4 class="page-title">Văn bằng</h4>
        </div>
      </div>
    </div>
    <!-- end page title -->
    ...
    >
      <el-form
        ...
        label-width="150px"
      >
        <!-- <el-form-item label="Mã Văn bằng" prop="MaVanBang">
          <el-input
            v-model="formData.MaVanBang"
            type="text"
            size="small"
            :readonly="isEditor"
          ></el-input>
        </el-form-item> -->
        <el-form-item label="Tên Văn bằng" prop="TenVanBang">
          <el-input
            v-model="formData.TenVanBang"
            type="text"
            size="small"
          ></el-input>
        </el-form-item>
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm("Bạn có chắc chắn muốn xóa?", "Thông báo", {
        ...
        deleteVanBang(row.Id).then(data => {
          this.$message({
            type: "success",
    ...
    updateData() {
      this.$refs.formData.validate(valid => {
        if (valid) {
          if (this.isEditor == 0) {
            addVanBang(this.formData).then(data => {
              //console.log(data);
              this.getListData();
            });
          } else {
            updateVanBang(this.formData.Id, this.formData).then(data => {
              //console.log(data);
              this.getListData();
            });
          }
    ...
    renderData() {
      var _data = this.listData.filter(post => {
        return (
          post.TenVanBang.toLowerCase().includes(this.search.toLowerCase())
        );
      });
