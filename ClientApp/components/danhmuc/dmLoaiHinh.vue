  ...
  <div>
        ...
        <div class="page-title-box">
          <div class="page-title-right">
            <ol class="breadcrumb m-0">
              ...
              <li class="breadcrumb-item active">Loại hình</li>
            </ol>
          </div>
          <h4 class="page-title">Loại hình</h4>
        </div>
      </div>
    </div>
    <!-- end page title -->
    ...
    >
      <el-form
        ...
        label-width="150px"
      >
        <!-- <el-form-item label="Mã Loại hình" prop="MaLoaiHinh">
          <el-input
            v-model="formData.MaLoaiHinh"
            type="text"
            size="small"
            :readonly="isEditor"
          ></el-input>
        </el-form-item> -->
        <el-form-item label="Tên Loại hình" prop="TenLoaiHinh">
          <el-input
            v-model="formData.TenLoaiHinh"
            type="text"
            size="small"
          ></el-input>
        </el-form-item>
    ...
    return {
      ...
      formRules: {
        TenLoaiHinh: [
          {
            required: true,
            message: "Vui lòng nhập Tên Loại hình",
            trigger: "blur"
          }
        ]
      },
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm("Bạn có chắc chắn muốn xóa?", "Thông báo", {
        ...
        deleteLoaiHinh(row.Id).then(data => {
          this.$message({
            type: "success",
      ...
      this.$refs.formData.validate(valid => {
        if (valid) {
          if (this.isEditor == 0) {
            ...
            addLoaiHinh(this.formData).then(data => {
              //console.log(data);
              this.getListData();
            });
          } else {
            //delete this.formData.LoaiHinh;
            updateLoaiHinh(this.formData.Id, this.formData).then(data => {
              //console.log(data);
              this.getListData();
            });
          }
    ...
    renderData() {
      var _data = this.listData.filter(post => {
        return (
          post.TenLoaiHinh.toLowerCase().includes(this.search.toLowerCase())
        );
      });
