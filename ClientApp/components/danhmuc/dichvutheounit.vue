<template>
  <div>
        ...
        <div class="page-title-box">
          <div class="page-title-right">
            <ol class="breadcrumb m-0">
              ...
              <li class="breadcrumb-item active">Quản lý dịch vụ theo đơn vị</li>
            </ol>
          </div>
          <h4 class="page-title">Quản lý dịch vụ theo đơn vị</h4>
        </div>
      </div>
    </div>
    <!-- end page title -->
        ...
        <div class="card-box table-responsive">
          ...
          <el-table :data="renderData()"
            ...
            <el-table-column prop="UnitId"
              ...
              <template slot-scope="scope">
                <text-highlight :queries="search" style="word-break: normal;">
                  {{ scope.row.UnitId ? scope.row.Unit.UnitName : " " }}
                </text-highlight>
              </template>
            </el-table-column>
    ...
    <el-dialog title="Cập nhật quản lý dịch vụ"
      ...
      <el-form :model="formData"
        ...
        <el-form-item label="Địa bàn" prop="UnitId">
    ...
    methods: {
      ...
      handleDelete(row) {
        this.$confirm("Bạn có chắc chắn muốn xóa?", "Thông báo", {
          ...
          deleteDichvuunit(row.Id).then(data => {
            this.$message({
              type: "success",
      ...
      updateData() {
        this.$refs.formData.validate(valid => {
          if (valid) {
            if (this.isEditor == 0) {
              addDichvuunit(this.formData).then(data => {
                //console.log(data);
                if (data != null && data != "") {
                  this.$message({
                    type: "success",
              ...
              updateDichvuunit(this.formData).then(data => {
                //console.log(data);
                if (data != null && data != "") {
                  this.$message({
                    type: "success",
      ...
      renderData() {
        var _data = this.listData.filter(post => {
          var id = post.DichVuId;
          return post.DichVuId;
