  ...
  <div>
        ...
        <div class="page-title-box">
          <div class="page-title-right">
            <ol class="breadcrumb m-0">
              ...
              <li class="breadcrumb-item active">Ngoại ngữ</li>
            </ol>
          </div>
          <h4 class="page-title">Ngoại ngữ</h4>
        </div>
      </div>
    </div>
    <!-- end page title -->
    ...
    >
      <el-form
        ...
        label-width="150px"
      >
        <!-- <el-form-item label="Mã Ngoại ngữ" prop="MaNgoaiNgu">
          <el-input
            v-model="formData.MaNgoaiNgu"
            type="text"
            size="small"
            :readonly="isEditor"
          ></el-input>
        </el-form-item> -->
        <el-form-item label="Tên Ngoại ngữ" prop="TenChungChi">
          <el-input
            v-model="formData.TenChungChi"
            type="text"
            size="small"
          ></el-input>
        </el-form-item>
    ...
    return {
      ...
      formRules: {
        TenChungChi: [
          {
            required: true,
            message: "Vui lòng nhập Tên Ngoại ngữ",
            trigger: "blur"
          }
        ]
      },
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm("Bạn có chắc chắn muốn xóa?", "Thông báo", {
        ...
        deleteNgoaiNgu(row.Id).then(data => {
          this.$message({
            type: "success",
    ...
    updateData() {
      this.$refs.formData.validate(valid => {
        if (valid) {
          if (this.isEditor == 0) {
            addNgoaiNgu(this.formData).then(data => {
              //console.log(data);
              this.getListData();
            });
          } else {
            updateNgoaiNgu(this.formData.Id, this.formData).then(data => {
              //console.log(data);
              this.getListData();
            });
          }
    ...
    renderData() {
      var _data = this.listData.filter(post => {
        return (
          post.TenChungChi.toLowerCase().includes(this.search.toLowerCase())
        );
      });
