  ...
  <div>
    ...
    >
      <el-form
        ...
        label-width="150px"
      >
        <!-- <el-form-item label="Mã Lĩnh vực" prop="MaLinhVuc">
          <el-input
            v-model="formData.MaLinhVuc"
            type="text"
            size="small"
            :readonly="isEditor"
          ></el-input>
        </el-form-item> -->
        <el-form-item label="Tên Lĩnh vực" prop="TenLinhVuc">
          <el-input
            v-model="formData.TenLinhVuc"
            type="text"
            size="small"
          ></el-input>
        </el-form-item>
    ...
    return {
      ...
      formRules: {
        TenLinhVuc: [
          {
            required: true,
            message: "Vui lòng nhập Tên Lĩnh vực",
            trigger: "blur"
          }
        ]
      },
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm("Bạn có chắc chắn muốn xóa?", "Thông báo", {
        ...
        deleteLinhVuc(row.Id).then(data => {
          this.$message({
            type: "success",
    ...
    updateData() {
      this.$refs.formData.validate(valid => {
        if (valid) {
          if (this.isEditor == 0) {
            addLinhVuc(this.formData).then(data => {
              //console.log(data);
              this.getListData();
            });
          } else {
            updateLinhVuc(this.formData.Id, this.formData).then(data => {
              //console.log(data);
              this.getListData();
            });
          }
    ...
    renderData() {
      var _data = this.listData.filter(post => {
        return (
          post.TenLinhVuc.toLowerCase().includes(this.search.toLowerCase())
        );
      });
