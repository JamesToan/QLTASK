      ...
      <div class="col-12">
        <div class="card-box table-responsive">
          ...
          <el-table :data="renderData()"
            ...
            <el-table-column prop="NguoiTao"
                             label="Người tạo"
                             width="180"
                             align="center"
                             sortable>
              <template slot-scope="scope">
                {{ scope.row.NguoiTaoId ? scope.row.User.FullName : "" }}
              </template>
            </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      changeDiaban(val) {
        ...
        console.log(this.formData2.UnitId);
      ...
      getListData() {
        ...
        else if (this.isAdvantage == true){
          getycdiaban(this.DiaBanId, this.NhanSuId).then(data => {
            this.listData = data;
            this.total = data.length;
            this.loading = false;
