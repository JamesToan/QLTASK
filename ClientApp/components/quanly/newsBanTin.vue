  ...
  <div>
    ...
    <div class="row">
      <div class="col-12">
        <div class="page-title-box">
          ...
          <h4 class="page-title">Quản lý Bản tin</h4>
        </div>
      </div>
    </div>
    <!-- end page title -->
      ...
      <div class="col-12">
        <div class="card-box table-responsive">
          ...
          >
            ...
            <el-table-column prop="TieuDe" label="Tiêu đề" width="300">
              <template slot-scope="scope">
                <text-highlight
                  :queries="search"
                  style="word-break: normal; font-weight: bold;"
                >
                  {{ scope.row.TieuDe }}
                </text-highlight>
              </template>
            </el-table-column>
            <el-table-column prop="TomTat" label="Tóm tắt" min-width="200">
              <template slot-scope="scope">
                <text-highlight :queries="search" style="word-break: normal;">
                  {{ formatTomTat(scope.row.TomTat) }}
                </text-highlight>
              </template>
            </el-table-column>
            <el-table-column
    ...
    >
      <el-form
        ...
        label-width="140px"
        class="m-auto"
        ><el-form-item label="Chuyên mục" prop="ChuyenMucId" v-show="0">
          <el-select
        ...
        <el-form-item label="Tóm tắt" prop="TomTat">
          <el-input
        ...
        <el-form-item label="Tác giả" prop="TacGia">
          <el-input
...
export default {
  ...
  methods: {
    ...
    handleActive(index, row) {
      if (this.RoleId == this.roleActive) {
        activeNewsBanTin(row).then(data => {
          //console.log(data);
          this.$message({
            type: "success",
            message: row.NguoiDuyetId
              ? "Hủy duyệt Bản tin thành công!"
              : "Duyệt Bản tin thành công!"
          });
          this.getListData();
        });
      } else {
        this.$message.error("Bạn không có quyền duyệt Bản tin!");
      }
    },
    ...
    handleDelete(row) {
      this.$confirm("Bạn có chắc chắn muốn xóa?", "Thông báo", {
        ...
        deleteNewsBanTin(row.Id).then(data => {
          this.$message({
            type: "success",
    ...
    getListData() {
      ...
      if (this.RoleId == 2) {
        //Quản lý
        selectNewsBanTin(this.ChuyenMucIdFilter).then(data => {
          this.listData = data;
    ...
    updateData() {
      this.$refs.formData.validate(valid => {
        if (valid) {
          ...
          //console.log(this.formData.HinhAnh);
          if (this.isEditor == 0) {
            addNewsBanTin(this.formData).then(data => {
              //console.log(data);
              this.$message({
                type: "success",
            ...
            updateNewsBanTin(this.formData).then(data => {
              //console.log(data);
              this.$message({
                type: "success",
    ...
    renderData() {
      var _data = this.listData.filter(post => {
        return (
          ...
          post.TomTat.toLowerCase().includes(this.search.toLowerCase())
        );
      });
  ...
  created() {
    getCauHinh("VaiTroDuyetBanTin").then(data => {
      if (data) {
    ...
    getListDanhMucNews().then(data => {
      if (data) {
        this.ListDMChuyenMuc = data.DMChuyenMucNews;
      }
    });
