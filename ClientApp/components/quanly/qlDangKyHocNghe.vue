<template>
  <div>
        ...
        <div class="page-title-box">
          <div class="page-title-right">
            <ol class="breadcrumb m-0">
              ...
              <li class="breadcrumb-item active">Đăng ký học nghề</li>
            </ol>
          </div>
          <h4 class="page-title">Quản lý Đăng ký học nghề</h4>
        </div>
      </div>
    </div>
    <!-- end page title -->
        ...
        <div class="card-box table-responsive">
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <text-highlight :queries="search" style="word-break: normal;">
                  {{ scope.row.DaoTaoNghe.TenKhoaHoc }}
                </text-highlight>
              </template>
            </el-table-column>
            <el-table-column
    ...
    <el-dialog
      title="Quản lý Đăng ký học nghề"
    ...
    >
      <el-form
        ...
        size="small"
      >
        <el-form-item label="Đào tạo nghề" prop="DaoTaoNgheId">
          <el-select
            v-model="formData.DaoTaoNgheId"
            placeholder="Chọn đào tạo nghề"
            class="w-100"
          >
            <el-option
              v-for="item in ListDaoTaoNghe"
              :key="item.Id"
...
export default {
    ...
    return {
      ...
      formRules: {
        DaoTaoNgheId: [
          {
            required: true,
            message: "Vui lòng chọn đào tạo nghề",
            trigger: "blur"
          }
        ],
        HoTen: [
  ...
  methods: {
    ...
    handleActive(index, row) {
      ...
      activeDangKyHocNghe(row).then(data => {
        //console.log(data);
        this.$message({
          type: "success",
          message: row.XacThuc
    ...
    handleDelete(row) {
      this.$confirm("Bạn có chắc chắn muốn xóa?", "Thông báo", {
        ...
        deleteDangKyHocNghe(row.Id).then(data => {
          this.$message({
            type: "success",
    ...
    handleExport() {
      import("../../vendor/Export2Excel").then(excel => {
        ...
        console.log(data);
        var filename = "DSHocNghe_" + moment().format("YYYYMMDD_hhmmss");
    ...
    updateData() {
      this.$refs.formData.validate(valid => {
        if (valid) {
          if (this.isEditor == 0) {
            updateDangKyHocNghe(this.formData).then(data => {
              //console.log(data);
              this.$message({
                type: "success",
    ...
    renderData() {
      var _data = this.listData.filter(post => {
        return (
          ...
          post.DaoTaoNghe.TenKhoaHoc.toLowerCase().includes(
  ...
  created() {
    ...
    getListDanhMucSVL().then(data => {
      if (data) {
        ...
        this.ListDMDaoTaoNghe = data.DMDaoTaoNghe;
      }
    });
    selectDaoTaoNghe().then(data => {
      if (data) {
        this.ListDaoTaoNghe = data;
      }
    });
  }
};
