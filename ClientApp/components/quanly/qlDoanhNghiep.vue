  ...
  <div>
      ...
      <div class="col-12">
        <div class="card-box table-responsive">
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <text-highlight :queries="search" style="word-break: normal;">
                  {{ scope.row.TenDoanhNghiep }}
                </text-highlight>
              </template>
            </el-table-column>
            <!-- <el-table-column prop="MaSoThue" label="Mã số thuế" width="150">
              <template slot-scope="scope">
                <text-highlight :queries="search" style="word-break: normal;">
                  {{ scope.row.MaSoThue }}
                </text-highlight>
            ...
            <el-table-column prop="KhuCumCN" label="Khu cụm CN" width="120">
              <template slot-scope="scope">
                {{ scope.row.KhuCumCN ? scope.row.KhuCumCN.TenKhuCumCN : "" }}
              </template>
            </el-table-column>
    ...
    >
      <el-form
        ...
        :disabled="!allowEdit"
      >
        <el-form-item label="Tên doanh nghiệp" prop="TenDoanhNghiep">
          <el-input
        ...
        <el-row>
          ...
          <el-col :span="12"
            ><el-form-item label="Loại hình" prop="LoaiHinhId">
              <el-select
        ...
        <el-form-item label="Ngành kinh tế" prop="NganhKinhTeId">
          <el-select
          ...
          >
            <el-option
              v-for="item in ListDMNganhKinhTe"
              :key="item.Id"
        ...
        <el-form-item label="Kinh doanh sản xuất" prop="KinhDoanhSanXuat">
          <el-input
            v-model="formData.KinhDoanhSanXuat"
            type="text"
            size="small"
          ></el-input>
        </el-form-item>
        <el-row>
          <el-col :span="12"
            ><el-form-item label="Năm hoạt động" prop="NamHoatDong">
              <el-input
                v-model="formData.NamHoatDong"
                type="number"
                size="small"
          ...
          <el-col :span="12">
            <el-form-item label="Số lao động" prop="TongSoLaoDong">
              <el-input
        ...
        <el-row>
          <el-col :span="12"
            ><el-form-item label="Khu cụm CN" prop="KhuCumCNId">
              <el-select
                v-model="formData.KhuCumCNId"
                placeholder="Chọn khu cụm công nghiệp"
                class="w-100"
              >
                <el-option
                  v-for="item in ListDMKhuCumCN"
                  :key="item.Id"
                  :label="item.TenKhuCumCN"
                  :value="item.Id"
                >
                </el-option>
              </el-select>
...
export default {
  ...
  methods: {
    ...
    formatJson(filterVal, jsonData) {
      return jsonData.map(data =>
        filterVal.map(f => {
          ...
          if (f == "LoaiHinh") {
            return result ? result.TenLoaiHinh : "";
          }
          if (f == "NganhKinhTe") {
            return result ? result.TenNganhKinhTe : "";
          }
          if (f == "KhuCumCN") {
            return result ? result.TenKhuCumCN : "";
          }
          if (f == "Huyen") {
    ...
    handleActive(index, row) {
      ...
      activeDoanhNghiep(row).then(data => {
        //console.log(data);
        this.$message({
          type: "success",
          message: row.XacThuc
    ...
    handleDelete(row) {
      this.$confirm("Bạn có chắc chắn muốn xóa?", "Thông báo", {
        ...
        deleteDoanhNghiep(row.Id).then(data => {
          this.$message({
            type: "success",
    ...
    renderData() {
      var _data = this.listData.filter(post => {
        return post.TenDoanhNghiep.toLowerCase().includes(
  ...
  created() {
    ...
    getListDanhMucSVL().then(data => {
      if (data) {
        ...
        this.ListDMKhuCumCN = data.DMKhuCumCN;
