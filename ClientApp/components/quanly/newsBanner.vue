      ...
      <div class="col-12">
        <div class="card-box table-responsive">
          ...
          >
            ...
            <el-table-column prop="TieuDe" label="Tiêu đề" width="220">
              <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    handleActive(index, row) {
      if (this.RoleId == this.roleActive) {
        activeNewsBanner(row).then(data => {
          //console.log(data);
          this.$message({
            type: "success",
            message: row.NguoiDuyetId
              ? "Hủy duyệt banner thành công!"
              : "Duyệt banner thành công!"
          });
          this.getListData();
        });
      } else {
        this.$message.error("Bạn không có quyền duyệt banner!");
      }
    },
    ...
    handleDelete(row) {
      this.$confirm("Bạn có chắc chắn muốn xóa?", "Thông báo", {
        ...
        deleteNewsBanner(row.Id).then(data => {
          this.$message({
            type: "success",
    ...
    getListData() {
      ...
      if (this.RoleId == 2) {
        //Quản lý
        selectNewsBanner(this.ChuyenMucIdFilter).then(data => {
          this.listData = data;
    ...
    updateData() {
      this.$refs.formData.validate(valid => {
        if (valid) {
          ...
          if (this.isEditor == 0) {
            addNewsBanner(this.formData).then(data => {
              //console.log(data);
              this.$message({
                type: "success",
            ...
            updateNewsBanner(this.formData).then(data => {
              //console.log(data);
              this.$message({
                type: "success",
  ...
  created() {
    getCauHinh("VaiTroDuyetBanner").then(data => {
      if (data) {
    ...
    getListDanhMucNews().then(data => {
      if (data) {
        this.ListDMChuyenMuc = data.DMChuyenMucBanner;
      }
    });
