...
namespace coreWeb.Controllers
{
    ...
    public class NewsVideoController : ControllerBase
    {
        ...
        public NewsVideoController(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        [HttpPost]
        public IActionResult Add([FromBody] NewsVideo model)
        {
            try
            {
                var user = new UserClaim(HttpContext);
        ...
        [HttpPost]
        public IActionResult Update([FromBody] NewsVideo model)
        {
            try
            {
                var user = new UserClaim(HttpContext);
                if (user.RoleId == 1 || user.RoleId == 2)
                {
                    var result = _context.NewsVideo.SingleOrDefault(e => e.Id == model.Id);
        ...
        [HttpPost]
        public IActionResult Active([FromBody] NewsVideo model)
        {
            try
            {
                var user = new UserClaim(HttpContext);
        ...
        public IActionResult Delete(int id)
        {
            try
            {
                var result = _context.NewsVideo.SingleOrDefault(e => e.Id == id);
                if (result != null)
                {
